...
def make_random_test_in():
    ...
    while (action_num < num_actions):
        ...
        action = choose_action() ## sell and buy got deadlock between them...
        if action == "C":
            num_of_orderIDs = len(client_orderID[client_thread])
            if num_of_orderIDs == 0:
                continue
            chosen_order_id = random.choice(client_orderID[client_thread])
        ...
        client_orderID[client_thread].append(order_id) ## between buy and sell still got deadlock!!
        order_id += 1
        action_num += 1
    lines_to_be_written.append("x")
    with open("random-test_temp.in", "w") as rt_f:
